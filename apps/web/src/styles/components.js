...
const componentStyles = `
  ...
  .ai-card-glow {
    position: absolute;
    inset: 0;
    background: radial-gradient(ellipse at 85% 15%, rgba(196,164,107,0.18) 0%, transparent 55%);
    pointer-events: none;
  }
  ...
  .ai-loading-bar:last-child { margin-bottom: 0; }

  /* ── PROVIDER MATCH BADGE ───────────────── */
  .provider-match-badge {
    margin-top: 6px;
    font-size: 10px;
    font-weight: 600;
  ...
  .provider-picker-card.selected .provider-match-badge {
    color: ${colors.brassLight};
  }
`;
